...
if f"results_{dataset_name}_{model_name}_rep_att.pickle" in os.listdir():
    exit(1)
...
# else:
path_dir = f"answers_multilex_replication/{model_name}/"
iter = 0

evaluation(path_dir, results, model_name, "None", len(dataset))
iter += 1

import pickle


pickle.dump(results, open(f"results_{dataset_name}_{model_name}_rep_att.pickle", "wb"))

results = pickle.load(open(f"results_{dataset_name}_{model_name}_rep_att.pickle", "rb"))
