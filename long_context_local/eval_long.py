...
if f"results_{dataset_name}_{model_name}.pickle" in os.listdir():
    exit(1)
...
def load_predicted_data(path):
    ...
    for file in ordered_files:
        if model_name == "llama3.1":
            predicted.append(json.load(open(path+file, "r"))[2]["content"])
            print(path, file) if len(json.load(open(path+file, "r"))[2]["content"]) < 1 else 1
        else:
            predicted.append(json.load(open(path+file, "r"))["conversation"]["content"])
            print(path, file) if len(json.load(open(path+file, "r"))["conversation"]["content"]) < 1 else 1
...
def evaluation(path_data, results, model, prompt_type, limit):
    if "eurlexsum" in dataset_name:
        predicted = load_predicted_data(path_data)
        if len(dataset_name.split("_")) <= 1:
            predicted = predicted[split][:limit]
    else:
...
#     path_dir = f"answers_longcontext/{dataset_name_aux}_{model_name}"
# else:
path_dir = f"answers_longcontext/{dataset_name}_{model_name}"
iter = 0

for prompt_type in os.listdir(f"{path_dir}"):
    path_data = f"{path_dir}/{prompt_type}/"
    evaluation(path_data, results, model_name, prompt_type, len(dataset))
    iter += 1
    print(f"Iteration {iter}/{3}")


import pickle


pickle.dump(results, open(f"results_{dataset_name}_{model_name}.pickle", "wb"))

results = pickle.load(open(f"results_{dataset_name}_{model_name}.pickle", "rb"))
