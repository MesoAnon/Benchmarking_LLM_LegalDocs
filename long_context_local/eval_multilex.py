...
def load_predicted_data(path):
    ...
    for file in ordered_files:
        predicted.append(json.load(open(path+file, "r"))["summary"])
        print(path, file) if len(json.load(open(path+file, "r"))["summary"]) < 1 else 1
...
# else:
path_dir = f"answers_multilex/{model_name}/"
iter = 0

for selection_type in os.listdir(f"{path_dir}"):
    path_data = f"{path_dir}/{selection_type}/"
    print(selection_type)
    evaluation(path_data, results, model_name, selection_type, len(dataset))
    iter += 1
